...
describe("Verify  product field for max validation ",function(){
    this.retries(2)

    it("product field with max validation",async()=> {
        ...
        await browser.pause(3000);
        await Studentpage.addproducttoinvoice(signinData.invoiceprodutwithmaxvalue);
        await browser.pause(3000);
        const message=await $("//p[@id='name-helper-text']");
        await expect(message).toHaveTextContaining("Cannot be more than 50 characters");
        await browser.pause(3000);
    ...
    after(async()=>{
        await browser.saveScreenshot("./ScreenshotsforAlis/Verifyinvoiceproductfieldformaxvalidation.png")
    })
})